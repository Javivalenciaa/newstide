...
# GSC — optional. Set GOOGLE_ACCESS_TOKEN or GSC_SERVICE_ACCOUNT_JSON.
...
# ── DEDUPLICATION ─────────────────────────────────────────────────────────────
# ── TOPIC CLUSTERS (deterministic, computed on read — no DB column needed) ────
# Mirrors finance_pipeline.py's topic_cluster_key(), adapted for English titles.
# Used by cluster_aware_reorder() below and by the /en/topics/[cluster] and
# /temas/[cluster] pillar pages (which apply the identical logic in TypeScript).
_CLUSTER_STOP = {
...
def topic_cluster_key(text: str) -> str:
    base = slugify(text)
    tokens = [t for t in base.split("-") if len(t) > 2 and t not in _CLUSTER_STOP]
    return "-".join(tokens[:5])

def cluster_aware_reorder(pool: list[str], recent_articles: list[dict]) -> list[str]:
    """Boost candidates whose topic cluster already has 2-5 recent articles (building
    real topical depth) over brand-new clusters or already-saturated ones (6+).
    Does NOT change ARTICLES_PER_RUN itself — same fixed count per run, just a
    smarter pick order layered on top of the existing DataForSEO score order."""
    depth: dict[str, int] = {}
    for a in recent_articles:
        key = topic_cluster_key(a.get("title_en") or a.get("keyword", ""))
        if key:
            depth[key] = depth.get(key, 0) + 1

    def score(candidate: str) -> int:
        d = depth.get(topic_cluster_key(candidate), 0)
        if 2 <= d <= 5:
            return 2
        if d == 0:
            return 1
...
def is_duplicate_topic_trgm(candidate: str, threshold: float = 0.45) -> bool:
    """Deterministic canibalización pre-check via Postgres pg_trgm (match_similar_articles
    RPC), run before the GPT check below. Catches near-duplicate titles GPT-4o-mini
    sometimes misses at scale, and skips the GPT call entirely when it already matches."""
    try:
        res = supabase_client.rpc(
            "match_similar_articles",
            {"candidate_title": candidate, "sim_threshold": threshold},
        ).execute()
...
def build_candidate_pool(recent_articles: list[dict]) -> list[str]:
    ...
    unique = format_diversity_reorder(
        unique, [a.get("title_en") or a.get("keyword", "") for a in recent_articles]
    )
...
# ── PERSISTENT RELATED ARTICLES (sidebar) ─────────────────────────────────────
# Separate from fetch_related_articles()/inject_internal_links() above, which
# only feed the in-body link prompt. This scores candidates by category match +
# title token overlap + recency, and the top N are persisted to the
# `related_articles` jsonb column so the 4 article page.tsx can render a
# sidebar without an extra query on every page view.
def compute_related_articles(category: str, slug_en: str, title_en: str, limit: int = 5) -> list[dict]:
    try:
        res = (
            supabase_client.table("articles")
            .select("title, title_en, slug, slug_en, category, published_at")
            .not_.is_("slug_en", "null")
            .neq("slug_en", slug_en)
            .order("published_at", desc=True)
            .limit(200)
        ...
        print(f"  ⚠️  compute_related_articles fetch failed: {e}")
        return []
    if not candidates:
        return []

    title_tokens = {
        w for w in re.sub(r"[^a-z0-9\s]", "", title_en.lower()).split() if len(w) > 3
    }
    now = datetime.now(timezone.utc)

    def score(c: dict) -> float:
        s = 3.0 if c.get("category") == category else 0.0
        c_title = (c.get("title_en") or c.get("title") or "").lower()
        c_tokens = set(re.sub(r"[^a-z0-9\s]", "", c_title).split())
        s += len(title_tokens & c_tokens)
        pub = c.get("published_at")
        if pub:
            try:
                dt = datetime.fromisoformat(str(pub).replace("Z", "+00:00"))
                days = max((now - dt).days, 0)
                s += max(0.0, 1.0 - days / 90.0)
            except Exception:
                pass
        return s
...
# Pure Python, no external deps. (An unused TS twin of this, content-guardrails.ts,
# was archived — nothing ever imported it; this Python version is what runs.)
...
def save_article(
    ...
    data = {
        ...
        "related_articles": compute_related_articles(category, slug, title_en),
    }
