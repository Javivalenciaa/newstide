...
def cluster_aware_reorder(pool: list[str], recent_articles: list[dict]) -> list[str]:
    ...
    for a in recent_articles:
        key = topic_cluster_key(a.get("title") or a.get("keyword", ""))
        if key:
...
def is_duplicate_topic_trgm(candidate: str, threshold: float = 0.45) -> bool:
    """Deterministic canibalización pre-check via Postgres pg_trgm (match_similar_finance_articles
    RPC), run before the cluster-cooldown/GPT checks below. Catches near-duplicate titles
    those checks sometimes miss at scale, and skips the GPT call when it already matches."""
    try:
        res = supabase_client.rpc(
            "match_similar_finance_articles",
...
def inject_images(content: str, cover: dict | None, inline: dict | None) -> str:
    def img_md(img: dict) -> str:
        ...
        credit = f"Foto de {img['author']} en Unsplash"
...
# only feed the in-body link prompt. Scores candidates by category match +
# title token overlap + recency; top N persisted to `related_articles` jsonb
# so the EN/ES finance page.tsx can render a sidebar without an extra query.
_RELATED_STOP = {
    "para", "como", "cómo", "con", "sin", "de", "la", "el", "los", "las",
    "que", "una", "uno", "en", "por", "tu", "su", "usa", "estados", "unidos",
}

def compute_related_articles(category: str, slug: str, title: str, limit: int = 5) -> list[dict]:
    try:
        res = (
            supabase_client.table("finance_articles")
    ...
    title_tokens = {
        ...
        if len(w) > 3 and w not in _RELATED_STOP
    }
    now = datetime.now(timezone.utc)
    ...
    def score(c: dict) -> float:
        ...
        c_tokens = {
            w for w in re.sub(r"[^a-z0-9\s]", "", (c.get("title") or "").lower()).split()
            if len(w) > 3 and w not in _RELATED_STOP
        }
