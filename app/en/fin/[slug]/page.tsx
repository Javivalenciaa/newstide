...
const FIN_CAT_COLORS: Record<string, string> = {
  ...
  'Credit':        '#e8d5a3',
  'Side Hustles':  '#f0a050',
  // Real categories from finance_pipeline.py's detect_category() (FIN_CATEGORIES, Spanish)
  'Crédito':        '#e8d5a3',
  'Impuestos':      '#f0a050',
  'Ahorro':         '#6ecfca',
  'Presupuesto':    '#9b8cef',
...
function extractHowToSteps(content: string, title: string): Array<{ name: string; text: string }> {
  ...
  for (const section of sections) {
    ...
    if (!heading || /^(faq|frequently asked|conclusion|when this doesn't work)/i.test(heading)) continue
    let text = ''
...
export default async function FinanceArticlePage({
  ...
  const content = stripImageCredits(article.content_en || article.content)
  ...
  // by compute_related_articles() in finance_pipeline.py); only entries that
