    ...
    <main>
      ...
      <section className="hero">
        ...
        <div className="hero-content">
          <div className="hero-badge"><span className="badge-dot" />Tech · IA · Startups</div>
          <h1 className="hero-title">La <span className="grad">inteligencia</span><br/>que transforma<br/>el futuro</h1>
          <p className="hero-sub">Artículos de fondo, herramientas y tendencias para founders, developers y profesionales.</p>
          <div className="hero-tags">
            <span className="tags-label">Tendencias:</span>
            ...
            <Link href={`/articulo/${featured.slug}`} className="featured-card">
              ...
              <div className="featured-content">
                <div className="featured-meta">
                  ...
                  <span className="meta-label">Artículo destacado</span>
                </div>
                <h2 className="featured-title">{featured.title}</h2>
                <p className="featured-desc">{featured.excerpt}</p>
                <div className="featured-footer">
                  <strong>{featured.author}</strong>
                  <span>·</span><span>{formatDate(featured.published_at)}</span>
                  <span>·</span><span>{featured.reading_time} min</span>
                  <span className="read-btn">Leer artículo →</span>
                </div>
              </div>
            </Link>
          </div>
          ...
          <div className="articles-layout">
            <div className="articles-grid">
              {rest.slice(0,6).map((a, i) => (
                <Link href={`/articulo/${a.slug}`} key={a.id} className="article-card" style={{'--delay': `${i*0.1}s`} as React.CSSProperties}>
                  <div className="article-img" style={{background: a.image_gradient}}/>
            ...
            <aside className="sidebar">
              ...
              <div className="sidebar-widget newsletter-widget">
                ...
                <p className="widget-sub">Las mejores historias de la semana en tu inbox.</p>
                <input type="email" placeholder="tu@email.com" className="sidebar-input"/>
                <button className="sidebar-btn">Suscribirse gratis</button>
              </div>
            </aside>
          </div>
        </div>
          ...
          <div className="newsletter-card">
            ...
            <div className="nl-stats">
              {[['8.4k','Suscriptores'],['97%','Tasa apertura'],['0','Spam']].map(([n,l]) => (
                <div key={l} className="nstat"><div className="nstat-num">{n}</div><div className="nstat-label">{l}</div></div>
              ))}
            </div>
          </div>
        </div>
